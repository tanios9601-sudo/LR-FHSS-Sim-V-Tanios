...
class Fragment():
    def __init__(self, type, duration, channel, packet):
        ...
        self.id = id(self)
        self.collided = []
        # Peak number of simultaneous signals on this fragment's channel
        # (including itself). Used to enforce SIC limit: if peak_collisions > L,
        # this fragment is unrecoverable even after SIC cancellation.
        self.peak_collisions = 1
...
class Base():
    ...
    def check_collision(self, fragment):
        ...
        # After updating all collided lists, compute the true peak:
        # total simultaneous signals = existing collisions + fragment itself.
        # This must be done AFTER the loop, because during the loop
        # len(fragment.collided) grows incrementally and would give wrong values.
        if len(fragment.collided) > 0:
            total_simultaneous = len(fragment.collided) + 1  # +1 = fragment itself
            fragment.peak_collisions = max(fragment.peak_collisions, total_simultaneous)
            for f in fragment.collided:
                f.peak_collisions = max(f.peak_collisions, total_simultaneous)
...
class Node():
    def __init__(self, obw, headers, payloads, header_duration, payload_duration, transceiver_wait, traffic_generator):
        ...
        self.headers_transmitted = 0  # ← ajouter
    ...
    def transmit(self, env, bs):
        while 1:
            ...
            while next_fragment:
                ...
                if next_fragment.type == 'header':
                 self.headers_transmitted += 1   # ← ajouter ici
                if self.packet.success == 0:
                 bs.try_decode(self.packet, env.now)
                #if self.packet.success == 0:
                    #bs.try_decode(self.packet,env.now)
...
class Base():
    def __init__(self, obw, threshold):
        ...
        self.packets_received = {}
        self.threshold = threshold
        # Total number of fragments that experienced at least one collision.
        # A fragment is counted even if later recovered by SIC (peak_collisions > 1).
        self.total_collided_fragments = 0
        self.total_collision_pairs = 0
    ...
    def finish_fragment(self, fragment):
        ...
        if len(fragment.collided) == 0:
            fragment.success = 1
        fragment.transmitted = 1
        # Count fragment if it experienced at least one collision at any point.
        # peak_collisions > 1 catches fragments whose collided list was later
        # cleared by SIC (try_decode) but did collide during transmission.
        if fragment.peak_collisions > 1:
            self.total_collided_fragments += 1
    ...
    def check_collision(self, fragment):
        for f in self.transmitting[fragment.channel]:
            ...
            self.total_collision_pairs += 1  # ← une paire détectée ici
