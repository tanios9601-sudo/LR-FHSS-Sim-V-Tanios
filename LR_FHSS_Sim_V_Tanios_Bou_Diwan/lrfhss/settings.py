...
class Settings():
    def __init__(self, number_nodes=80000//8, simulation_time=60*60, payload_size = 10, headers = 3, header_duration = 0.233472, payloads = None, threshold = None, payload_duration = 0.1024,
        ...
        # SIC limit L: max number of simultaneous overlapping signals that can
        # be resolved. None = no limit (ideal SIC). Set e.g. sic_limit=2 to
        # discard any fragment that experienced 3+ simultaneous signals.
        self.sic_limit = sic_limit
        # gamma: SIC efficiency (Dumas et al. model).
        # w_l = gamma^(l-1) = probability to recover a fragment
        # that experienced l simultaneous signals.
        # gamma=1.0 -> perfect SIC (original behaviour).
        self.gamma = gamma
